...
```{r}
con_flax <- dbConnect(RSQLite::SQLite(), "data/ftms_flax_neg.sqlite")
```

Loading the sql database of Dynlib qtof negative

```{r}
con_dyL <- dbConnect(RSQLite::SQLite(), "data/DyLib_FTMS_neg_or.sqlite")
```
...
```{r}
# Create new database
con_merged <- dbConnect(SQLite(), "data/DYL_FLAX_FTMSneg.sqlite")
```
...
# Usage
merge_databases(
  main_db   = "data/DyLib_FTMS_neg_or.sqlite",
  add_db    = "data/ftms_flax_neg.sqlite",
  output_db = "data/DYL_FLAX_FTMSneg.sqlite"
)
...
```{r}
con_merged <- dbConnect(SQLite(), "data/DYL_FLAX_FTMSneg.sqlite")
adding_Names_SQL(con_merged, result)
```
