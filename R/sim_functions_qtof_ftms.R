...
sim_final_final <- function(st_sps, dy_sps, 
                            collision_energy_qtof,
                            threshold = 0.8, ppm = 0, tolerance = 0.005) {
  
  library(MetaboAnnotation)
  
  filenames <- dataOrigin(st_sps)
  dda <- stringr::str_extract(filenames, "DDA[0-9]")
  spectraData(st_sps)$dda <- dda
  
  st_filtered <- st_sps[st_sps$dda == collision_energy_qtof]
  

  if (length(st_filtered) == 0 || length(dy_sps) == 0) {
    return(data.frame())
  }
